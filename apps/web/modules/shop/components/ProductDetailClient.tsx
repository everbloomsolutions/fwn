...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const [selectedUnit, setSelectedUnit] = useState<string>(
    inStock?.unit || active[0]?.unit || product.variants[0]?.unit
  );
  ...
  return (
    <Container maxWidth="xl" className="py-6 sm:py-12 lg:py-16">
      ...
      <section className="mt-12 rounded-2xl border border-border bg-surface p-4 sm:p-6">
        <Heading level="h2" size="compact" className="mb-4">
          Reviews
        </Heading>
        {product.reviewCount ? (
          <Text className="text-text-muted">{product.reviewCount} reviews · Average {product.rating?.toFixed(1)} ★</Text>
        ) : (
          <Text className="text-text-muted">No reviews yet. Be the first to review this product.</Text>
        )}
      </section>
